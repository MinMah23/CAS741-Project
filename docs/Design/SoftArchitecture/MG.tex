...
\item[\refstepcounter{acnum} \actheacnum \label{acODEs}:] How the governing Ordinary Differential Equations (ODEs) are defined using the input parameters.
\item[\refstepcounter{acnum} \actheacnum \label{acSolver}:] The algorithm used for solving the equations of motions ODEs.
\item[\refstepcounter{acnum} \actheacnum \label{acPlot}:] The implementation of plotting the output.
\end{description}
...
\item[\refstepcounter{ucnum} \uctheucnum \label{ucGoal}:] The goal of the system is to calculate the position of cart and the angle of pendulum.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucODEstructure}:] The ODEs for the cart and pendulum motions can be defined using parameters defined in the input parameters module.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucEqM}:] How the equations of motions of the cart and the pendulum are defined using the specification of the system.
...
Note that \mref{mHH} is a commonly used module and is already implemented by the operating system.  It will not be reimplemented.  Similarly, \mref{mODEs}, \mref{mSolver} and \mref{mPlot} are already available in Python and will not be reimplemented.
...
\multirow{3}{0.3\textwidth}{Behaviour-Hiding Module} & Input Parameters Module\\
...
R2 & \mref{mParams}\\
R3 & \mref{mSolver}\\
R4 & \mref{mHH}, \mref{mOutput}, \mref{mPlot}\\
...
\begin{description}
\item[Secrets:] The algorithm to solve a system of first or higher orders ODEs initial
  value problem from a given starting time until the given event function shows termination.
